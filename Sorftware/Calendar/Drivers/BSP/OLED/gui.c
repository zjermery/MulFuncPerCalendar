...
{ 
	while (*str != '\0') {
        uint16_t unicode = *str; // 获取当前字符的 Unicode 值
        if (unicode > 128) {
            // 显示中文字符
			GUI_ShowFont16(x,y,str,mode);
            x += 8;
            str += 1; // UTF-16 每个字符占用两个字节
        } 
		else 
		{
            // 显示英文字符
            if (unicode < 128) {
                GUI_ShowChar(x, y, (uint8_t)unicode, 16, mode);
				x += 8; 
            }
            str += 1; // 对于英文字符，假设也是 UTF-16 编码，但高位字节为 0
        }
		if (x > WIDTH - 16) {
			x = 0;
			y += 16;
